...
other_engine_channels = {}
other_engine_volumes = {}
...
def updateOtherEngineSound(engine_volumes):

    global other_engine_channels
    global other_engine_volumes
    ...
    # ---------------------------------------------
    # Создаём каналы для новых танков
    # ---------------------------------------------

    for tank_id in engine_volumes:

        if tank_id not in other_engine_channels:

            # Каналы 2, 3, 4, 5...
            channel_number = 2 + len(
                other_engine_channels
            )

            other_engine_channels[tank_id] = (
                pygame.mixer.Channel(channel_number)
            )

            other_engine_volumes[tank_id] = 0.0

            other_engine_channels[tank_id].play(
                sounds['Driving'],
                loops=-1
            )
    ...
    for tank_id, channel in other_engine_channels.items():

        target_volume = engine_volumes.get(
            tank_id,
            0.0
        )

        current_volume = other_engine_volumes[tank_id]
        ...
        other_engine_volumes[tank_id] = current_volume
