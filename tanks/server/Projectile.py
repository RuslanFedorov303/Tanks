...
class Projectile:
    def __init__(projectile,
        ...
        projectile.x_coord_speed = x_coord_speed * speed
        projectile.y_coord_speed = y_coord_speed * speed
        projectile.rotate        = rotate
        projectile.damage      = damage
        projectile.penetration = penetration
        projectile.comand      = comand
        projectile.tanks_list = tanks_list
        projectile.surface = pygame.transform.rotate(pygame.Surface((10, 10)), rotate)
        projectile.rect  = projectile.surface.get_rect()
        projectile.mask  = pygame.mask.from_surface(projectile.surface)
        projectile.colide = False



    def _projectileMove(projectile):
        projectile.x_coord += projectile.x_coord_speed
        projectile.y_coord += projectile.y_coord_speed
    ...
    def _projectileColideTanks(projectile):
        for tank in projectile.tanks_list:
            if tank.comand == projectile.comand:
