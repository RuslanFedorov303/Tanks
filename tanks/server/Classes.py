...
class Tank:
    def __init__(self,
        ...
        self.projectiles   = projectiles  # Списки для взаемодействия
    ...
    def _gunShoot(self):
        ...
        if ('MOUSEBUTTONDOWN' in self.events
    ...
    # def tankHitboxesDraw(self, screen):
    #     for hitbox in hitboxes:
    #         hitbox



    def tankUpdate(self, events = tuple, screen = None):
        try:
            if not self.player_socket == None:
                self._socketPlayerReceivingEvent()
            else:
                self._localGameEventLoad(events)
